...
function favoriteImages() {
    ...
    for(var i=0; i<metadata.length; i++){
        ...
        for(var j=0; j<favArray.length; j++)
        {
            ...
            {
                checked = "checked";
                container.append("<figure class='box' id='" + i + "'> <figcaption>" + metadata[i].title + "</figcaption> <img class='expand' src='" + metadata[i].url + "'> <figcaption> Artist: " + metadata[i].artist + "</figcaption></figure>");   
                break;
            }
        }
    }
